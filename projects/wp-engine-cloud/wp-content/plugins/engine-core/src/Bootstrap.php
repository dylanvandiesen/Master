...
final class Bootstrap
{
    ...
    public static function init(string $pluginFile): void
    {
        ...
        self::$container->singleton(MigrationConfigLoader::class, static fn (): MigrationConfigLoader => new MigrationConfigLoader());
        self::$container->singleton('engine_core.migration_config', static fn (Container $c): array => $c->get(MigrationConfigLoader::class)->load(dirname(__DIR__) . '/config/migrations/schema-map.json'));
        self::$container->singleton(MigrationValidator::class, static fn (): MigrationValidator => new MigrationValidator());
        self::$container->singleton(MigrationRegistry::class, static fn (Container $c): MigrationRegistry => new MigrationRegistry(
            $c->get('engine_core.migration_config')['steps']
        ));
        self::$container->singleton(MigrationRunner::class, static fn (Container $c): MigrationRunner => new MigrationRunner(
            ...
            $c->get(MigrationValidator::class),
            (string) ($c->get('engine_core.migration_config')['latest'] ?? '1.0.0')
        ));
        ...
        add_action('init', static function (): void {
            try {
                ...
                self::$container?->get('engine_core.migration_config');
            } catch (ValidationException $exception) {
