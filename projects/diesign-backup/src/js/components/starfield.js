...
const { disconnect } = observeCanvasSize(canvas, () => {
  createStars(); // Your starfield's star layout reset
});
  ...
  instance.disconnect = observeCanvasSize(canvas, () => {
    if (!initialized) {
      createStars();
      initialized = true;
      requestAnimationFrame(() => readyResolve()); // Resolve only once, when canvas is really ready
    }
  }).disconnect;
