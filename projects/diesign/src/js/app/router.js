export function setupDynamicNavigation(options = {}) {
  ...
  async function openProjectPopover(projectId, { jobId = routeJobId } = {}) {
    ...
    popover._toggleHandler = (event) => {
      if (event.newState !== 'closed') {
        return;
      }

      if (location.pathname !== `${workPath}/${projectId}`) {
        return;
      }
  ...
  function loadPopoverModules() {
    popoverModulesPromise ||= Promise.all([
      import('../features/work/popover-media.js'),
      import('../features/work/popover-scroll-snap.js')
