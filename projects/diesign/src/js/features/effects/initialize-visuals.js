...
function createMeteorScheduleSync(instance) {
  return () => {
    if (!instance) {
      return;
    }
    ...
    if (HomeState.get()) {
      instance.startMeteorShowerSchedule({
        delay: 2500,
        sequence: [
    ...
    instance.stopMeteorShowerSchedule?.();
  };
}
...
export function initializeVisuals() {
  visualsReadyPromise ||= Promise.all([
    ...
    const syncMeteorSchedule = createMeteorScheduleSync(starfieldInstance);
    syncMeteorSchedule();
...
export function triggerIntroMeteorBurst() {
...
export function queueVisualInitialization() {
  requestAnimationFrame(() => {
