...
export default class PopoverDragScrollSnap {
  ...
  bindEvents = () => {
    ...
    const handlers = {
      ...
      onFigureMouseDown: this.onFigureMouseDown.bind(this),
    };
    ...
    // Vertical drag on figures
    if (this.config.verticalDragOnFigures) {
      this.body?.addEventListener('mousedown', handlers.onFigureMouseDown, { passive: false });
    }
  ...
  destroy = () => {
    ...
    if (this._boundHandlers) {
      ...
      if (this.config.verticalDragOnFigures) {
        this.body?.removeEventListener('mousedown', this._boundHandlers.onFigureMouseDown);
      }
    }
